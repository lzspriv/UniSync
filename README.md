...
1. 說明問題：校內公告分散在各系所與行政單位網站，使用者很難持續追蹤。
2. 展示首頁：登入、訂閱來源、搜尋選單、公告預覽。
3. 展示分類訂閱：勾選某個單位或公告類型並儲存。
4. 展示全域關鍵字：新增關鍵字並查看預覽命中公告。
5. 展示 Live Feed：說明它會整合分類訂閱與全域關鍵字命中。
6. 展示通知：以 Discord 或 Telegram 作為實際通知結果。
7. 展示自動化：說明 GitHub Actions 每小時爬取並更新 Supabase 與預覽快取。
8. 展示可維護性：打開 `university-config.json` 說明新增單位主要靠設定檔。
