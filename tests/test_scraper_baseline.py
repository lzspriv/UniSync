...
class ScraperParsingBaselineTests(unittest.TestCase):
    ...
    def test_parse_taiwan_date_supports_roc_date(self):
        date_text, summary = scraper_parsing.parse_taiwan_date("🏷️ 115年4月29日 招生公告")
    ...
    def test_parse_date_from_url_supports_wordpress_paths(self):
        result = scraper_parsing.parse_date_from_url(
    ...
    def test_article_matches_class_id_and_tag_selectors(self):
        ...
        self.assertTrue(scraper_parsing.article_matches_selector(article, ".pinned"))
        self.assertTrue(scraper_parsing.article_matches_selector(article, "#important"))
        self.assertTrue(scraper_parsing.article_matches_selector(article, "article"))
        self.assertFalse(scraper_parsing.article_matches_selector(article, ".normal"))


class ScraperHttpBaselineTests(unittest.TestCase):
    def test_build_request_options_uses_defaults(self):
        options = scraper_http.build_request_options()

        self.assertEqual(options["timeout"], 10)
        self.assertIs(options["headers"], scraper_http.REQUEST_HEADERS)
    ...
    def test_create_request_session_uses_legacy_adapter_only_for_pr_site(self):
        legacy_session = scraper_http.create_request_session("https://pr.ntnu.edu.tw/news/")
        normal_session = scraper_http.create_request_session("https://www.ntnu.edu.tw/")
